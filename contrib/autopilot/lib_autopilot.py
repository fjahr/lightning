...
long term goal is to generate a lightning network with good topological
properties.
...
class Autopilot():
    ...
    def __sample_from_pdf(self, pdf, k=21):
        ...
        keys, values = pdf.keys(), pdf.values()
        if k >= len(keys):
            return keys
        res = np.random.choice(keys, k, replace=False, p=values)
        return res
    ...
    def __get_uniform_pdf(self):
        ...
        In contrast to other methods there are no arguments for smoothing
    ...
    def __get_centrality_pdf(self, skew=False, smooth=False):
        """
        Produces a probability distribution which is proportional to nodes
        betweeness centrality scores.

        The betweeness centrality counts on how many shortest paths a node is
        connecting to those nodes will most likely make them even more central.
        However it is good for the node operating the autopilot as this node
        itself gets a position in the network which is close to central nodes.

        This distribution can be skewed and smoothed
        """
        self.__logger.info("""CENTRALITY_PDF: Try to generate a PDF proportional
                           to centrality scores""")
        pdf = {}
        cumsum = 0
        ...
        self.__logger.info("CENTRALITY_PDF: Generated pdf")

        if skew and smooth:
            self.__logger.info("""CENTRALITY_PDF: Won't skew and smooth \
                               distribution ignore both""")
            smooth = False
            skew = False
    ...
    def __get_rich_nodes_pdf(self, skew=False, smooth=False):
        ...
        sums of capacities after deriving a pdf. If one whishes the method
    ...
    def __get_long_path_pdf(self, skew=True, smooth=False):
        ...
        """
        if skew and smooth:
            self.__logger.info("""DECREASE DIAMETER: Can't skew and smooth \
                               distribution ignore smoothing""")
            smooth = False
        ...
        # TODO CD: https://github.com/ElementsProject/lightning/pull/1888#discussion_r217931040
    ...
    def __create_pdfs(self):
        # FIXME CD: https://github.com/ElementsProject/lightning/pull/1888#discussion_r217931100
        res = {}
    ...
    def calculate_proposed_channel_capacities(self, pdf, balance=1000000):
        # lnd uses 20k satoshi which seems reasonble
        minimal_channel_balance = 20000
        ...
        while needed_total_balance > balance and len(pdf) > 1:
            ...
            # FIXME CD: https://github.com/ElementsProject/lightning/pull/1888#discussion_r217931199
            k = [k for k, v in pdf.items() if v == min_val][0]
            self.__logger.info("""Not enough balance to open {} channels. \
                               Remove node: {} and rebalance pdf for channel \
                               balances""".format(len(pdf), k))
            del pdf[k]
            ...
            needed_total_balance = math.ceil(
                minimal_channel_balance / min_probability)
            self.__logger.info("""Need at least a balance of {} satoshi to open \
                               {} channels""".format(needed_total_balance,
                                                     len(pdf)))

        return pdf

    # FIXME CD: https://github.com/ElementsProject/lightning/pull/1888#discussion_r217931250
    def find_candidates(self, num_items=21, strategy=Strategy.DIVERSE,
                        percentile=None):
        self.__logger.info("""running the autopilot on a graph with {} nodes \
                           and {} edges.""".format(len(self.G.nodes()),
                                                   len(self.G.edges())))
        """
        Generates candidates with several strategies
        """
        sub_k = math.ceil(num_items / 4)
        self.__logger.info("""GENERATE CANDIDATES: Try to generate up to {} \
                           nodes with 4 strategies: (random, central, network \
                           Improvement, liquidity)""".format(num_items))
        ...
        more reasonable.
        My (renepickhardt) gut feeling is that several samples which are
        merged give us the best of all worlds where the other method would
        ...
        if strategy == Strategy.DIVERSE:
            for strategy, pdf in res.items():
                ...
                # FIXME CD: https://github.com/ElementsProject/lightning/pull/1888#discussion_r217931325
                candidates = candidates.union(set(tmp))
            ...
            for pdf in res.values():
                for k, v in pdf.items():
                    if k not in merged:
                        ...
                        merged[k] += v/denominator
            candidates = self.__sample_from_percentile(merged, percentile,
                                                       num_items)
        # FIXME CD: remove dead code
        """
        ...
        if len(candidates) > num_items:
            candidates = np.random.choice(list(candidates), num_items,
                                          replace=False)

        self.__logger.info("""GENERATE CANDIDATES: Found {} nodes with which \
                           channel creation is suggested""".format(
                               len(candidates)))
        return candidates


if __name__ == '__main__':
    print("""This lib needs to be given a network graph so you need to create \
          a wrapper""")
